...
##
## Holds multiples [AudioStreamPlayer] as child, and choose randomly one to play
...
## Add [param node] to [member _sounds] if it's an [AudioStreamPlayer].
...
func play() -> AudioStreamPlayer:
	if len(_sounds) == 0:
		...
		return
	var sound: AudioStreamPlayer = _sounds.pick_random().duplicate()
