...
##
## Holds multiples [AudioStreamPlayer2D] as child, and choose randomly one to play
...
## Add [param node] to [member _sounds] if it's an [AudioStreamPlayer2D].
...
func play() -> AudioStreamPlayer2D:
	if len(_sounds) == 0:
		...
		return
	var sound: AudioStreamPlayer2D = _sounds.pick_random().duplicate()
